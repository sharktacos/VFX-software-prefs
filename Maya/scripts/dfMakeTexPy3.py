...
class Main():
    def __init__(self):
        ...
        if not mc.optionVar(q='nMakeTx_makeTxPath'):
            mc.optionVar(sv=['nMakeTx_makeTxPath',    'h:/shared_assets/configs/makeTex/MakeTx_files'])
        ...
        self.pathTextField = mc.textFieldButtonGrp('myPathTextField', label='makeTx.exe path:', text = "h:/shared_assets/configs/makeTex/MakeTx_files", buttonLabel='Browse',  buttonCommand=Callback(self.browse, 'myPathTextField'), changeCommand=self.saveOptions, cw=[2, 350], w=600, p=self.optionLayout)
    ...
    def simpleAdditionlWidget(self, myLabel, flag):
        ...
        mc.textField('AdditionOptions', w=530, p=self.tempLayout, enable=True, insertText = '--fixnan box3 --monochrome-detect --constant-color-detect --opaque-detect --unpremult --prman')
    ...
    def convert(self, *args):
        ...
        for i, texFileIn in enumerate(myFileList):
            ...
            if (mc.checkBox('%s'%controlCheckbox, q = True, v = True)):
                LinearOption ='%s' '%s:' '%s ' '%s ' '%s' %('--compression ', cType,cLevel, '--colorconvert dt16 lnf --format exr -d half', AddOptions )
