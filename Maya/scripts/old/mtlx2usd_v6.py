...
def set_usd_input_connection(usd_shader, usd_type, mtlx_input_name, mtlx_output, nodegraph_shader):
    ...
    '''
    usd_shader: The USD shader (UsdShade.Shader object) to connect to.
    usd_type: The USD type of the input (e.g., "Color3f", "Float", Sdf.ValueTypeNames.Color3f).
    mtlx_input_name: The name of the input on the USD shader (e.g., "baseColor", "normal").
    mtlx_output: The name of the output on the NodeGraph node (e.g., "out", "outColor").
    nodegraph_shader: The USD NodeGraph (UsdShade.NodeGraph object) containing the output.
    '''
    ...
    '''
    outPort = nodegraph_shader.CreateOutput(mtlx_output, usd_type_name) # get output
    surfaceTerminal = usd_shader.GetInput(mtlx_input_name) # get input
    if surfaceTerminal: #Check if the input exists
        surfaceTerminal.ConnectToSource(outPort) # connect 
...
def materialx_to_usd(mtlx_filepath, usd_filepath, asset_name):
    ...
    for surf_elem in standard_surface_elem.findall("./input"):
        ...
        if mtlx_nodegraph and mtlx_surf_output:  # Check for connections to surface
            ...
            nodegraph_path = f"{material_path}/{mtlx_nodegraph}"
            #  /ASSET_MaterialX/mtl/carBody_SG/carBody_mtl/carBody_nodes
            ...
            # 5. Add Nodes in Nodegraph...
            nodegraph_elem = root.find("./nodegraph")
            if not nodegraph_elem:
                return
                
            #node_path = f"{material_path}/{mtlx_nodegraph}"
            
            # 6. Connect maps to Node Graph 
            #set_usd_input_connection(nodegraph_shader, usd_surf_type, mtlx_input_name, mtlx_surf_output, nodegraph_shader) 
            ...
            for node_elem in nodegraph_elem.findall("./*"): # Iterate through all elements under nodegraph
                if "nodedef" in node_elem.attrib: # Check if it has a nodedef attribute (i.e. that it is a node)
                    ...
                    #outNode = usd_node_shader.CreateOutput("out", sdf_node_type)
                ...
                for input_elem in node_elem.findall("./input"):
                    if "nodedef" in node_elem.attrib:
                        ...
                        if input_nodename:
                            ...
                            set_usd_input_connection(usd_node_shader, usd_node_type, input_name, input_nodename, usd_node_shader)
                            
                       
                            
                    
                    # 8. Populate outputs for Nodegraph terminal            
                    ...
                    if nodegraph_elem is not None:
                        for output_elem in nodegraph_elem.findall("./output"):  # Directly find <output> elements
                            ...
                            # Create USD shader from path
                            output_path = f"{nodegraph_path}/{output_nodename}"
                            ...
                            sdf_output_type = map_mtlx_type_to_sdf(output_type)
                            outNode = usd_output.CreateOutput('out', sdf_output_type)  #creates out from texture

    
                            NodeGraph_hub = nodegraph_shader.CreateOutput(output_name, sdf_output_type) #creates out in graph terminal
